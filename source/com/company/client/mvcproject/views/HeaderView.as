package com.company.client.mvcproject.views
{
	 ...
	 * 
	 * <p>Class is responsible for the display of the Header content in the swf application/site</p>
	 * 
 ...
 ****************************************************************************************/
		...
		/**
		 * textfield for displaying header text 
		 */		
 ...
 *************************************************************************************/
		 ...
		 */		
		public function HeaderView(aModel:IModel, aController:Object = null)
		{
		...
		{
			// build the header views background
			var bgWidth:Number = _model.getSwfDimensions().x - _leftMargin - _rightMargin;
			_headerBackground = DrawingUtils.drawRectangleSprite(bgWidth,90,{color:0x6699CC}, null, 15);
			DrawingUtils.set9GridScaling(_headerBackground, 15, false);
			addChild(_headerBackground);
			
			// add title for header
			_tfHeading = TextFieldUtils.getStandardTextField(false, false, false, false, true, "left", "advanced");
			_tfHeading.defaultTextFormat = ApplicationFonts.getStandardTextFormat();
			_tfHeading.htmlText = "<font color='#336699'><b>HEADER VIEW</b></font>";
			addChild(_tfHeading);
		...
		{
			_headerBackground.x = _leftMargin;
			_headerBackground.y = _topMargin;			
			_headerBackground.width = _model.getSwfDimensions().x - _leftMargin - _rightMargin;
		
			_tfHeading.x = _headerBackground.x + (_headerBackground.width - _tfHeading.width)/2;
			_tfHeading.y = _headerBackground.y + (_headerBackground.height - _tfHeading.height)/2;
						
			if(_tfHeading.x < _leftMargin + _headerBackground.x)
			{
				_tfHeading.x = _leftMargin + _headerBackground.x;
			}
		}
		...
		private function unbuildView():void
		{
			...
			_headerBackground.graphics.clear();			
			this.removeChild(_headerBackground);
			_headerBackground = null;
			
			// remove heading textfield
			this.removeChild(_tfHeading);
			_tfHeading = null;
